...
describe('concat', () => {
  ...
  it('should concat tony and baah, () => {
    const result = "tony".concat("baah");
    expect(result).to.equal("tonybaah");
  });

  it('should concat gugu and dube', () => {
    const result = "gugu".concat("dube");
    expect(result).to.equal("gugudube");
  });

    it('it should throw an error if either string is empty.', () => {
    expect(() => { concat('', '')}).to.throw('string cannot be empty');
  }):;
